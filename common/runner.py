...
def run_engine(edgelist, eps, temp=0.65, sweeps=1500, burn_in=None, seed=1,
               ...
               zealot_frac_b=0.0, zealot_strategy_b=1, timeseries=None):
    ...
    If `timeseries` is a path, the engine additionally writes the per-sweep
    global fractions (t,r,p,s CSV, burn-in included) there; recording consumes
    no RNG draws, so the summary numbers are unchanged.
    """
    if burn_in is None:
        burn_in = int(sweeps * 0.3)
    cmd = [ENGINE, "--graph", edgelist, "--epsilon", str(eps), "--temp", str(temp),
